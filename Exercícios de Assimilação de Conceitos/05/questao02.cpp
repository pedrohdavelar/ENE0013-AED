...
public:
  ...
  void addAfter(int, doubleNode*);  //Insere elemento após o nó apontado
...
doubleNode* doubleLinkedList::inList(int key)
{
  doubleNode *i;
...
void doubleLinkedList::print()
{
    ...
    std::cout << i->data;    //precisa imprimir o ultimo numero (next=0)
}
...
void doubleLinkedList::printReverse()
{
    ...
    std::cout << i->data;   //precisa imprimir o primeiro numero (previous=0)
}

void doubleLinkedList::addAfter(int info, doubleNode *p)
{
    doubleNode *tmp = new doubleNode(info, p, p->next); //cria o novo nó
    if (p==tail){
        p->next = tmp;                //primeiro no aponta para o no novo
        tail = tmp;
    } 
    else {
        p->next->previous = tmp;       //ultimo no aponta para o no novo
        p->next = tmp;                //primeiro no aponta para o no novo
    }
}
...
int main (){
    ...
    //doubleNode* test;
    //test = lista.inList(10);
    //std::cout << "test: " << test->data << std::endl;
    lista.addAfter(12,lista.inList(10));
    //lista.print();
    //std::cout << std::endl;
    lista.addAfter(13,lista.inList(12));
    lista.print();
    std::cout << std::endl;
    lista.printReverse();
    return 0; 

...
int main (){
  ...
  while(nomeInput != "S"){
    listaNomes.insert(nomeInput);
    std::cin >> nomeInput;
  } 

  listaNomes.inorder();

  std::cin >> funcInput >> nomeInput;
  while(funcInput != "S"){
    if(funcInput == "P"){
        if(listaNomes.search(nomeInput) == 0){
          std::cout << "nome nao encontrado" << std::endl;
        } else {
          std::cout << "nome encontrado" << std::endl;
        }
    }
    if(funcInput == "R"){
        listaNomes.findAndDeleteByCopying(nomeInput);
    }
    if(funcInput != "P" && funcInput != "R"){     

...
class meuGrafo : public Graph{

    public:
    //copia do construtor padrao
    meuGrafo(bool directed) : Graph(directed){}; 
    
    void leiaGrafo(){
        std::string input;
        std::cin >> input;
        while (input != "f"){
            //input = v -> adicionar vertice
            if (input == "v"){
                std::string nomeVertice;
                std::cin >> nomeVertice;
                this->addNode(nomeVertice);
            }
            if (input == "e"){
                std::string nomeVertice1, nomeVertice2;
                double peso;
                std::cin >> nomeVertice1 >> nomeVertice2 >> peso;
                this->addEdge(nomeVertice1, nomeVertice2, peso);
            }
            std::cin >> input;
        }
    ...
    void exibeCaminhoMinimo(std::string origem, std::string destino){
        std::map<std::string, std::string> antecessor;
        std::string vertice;
        std::stack<std::string> pilha;

        antecessor = this->findShortestPath(origem);
...
int main (){
    meuGrafo myGrafo(false);
    myGrafo.leiaGrafo();
    std::string origem,destino;
    std::cin >> origem >> destino;
    myGrafo.exibeCaminhoMinimo(origem, destino); 
    return 0;
}